...
const oddValues = (input) => {
  ...
  // Solution code here...
  return input.filter(input => input % 2 === 1);
...
const filterStringsWithVowels = (input) => {
  // Solution code here...
  let regex = /[aeiou]/;
  return input.filter(input => input.match(regex));
};
...
const getStatName = (input, minBaseStat) => {
  // Solution code here...
  return input.filter(x => x.baseStat > minBaseStat).map(s => s.stat.name);
};
...
const getCharactersWithoutChildren = (input) => {
  // Solution code here...
  return input.filter(stuff => !stuff.children);
};
// ------------------------------------------------------------------------------------------------
// CHALLENGE 7
//
...
const evenOddNumericValues = (input) => {
  // Solution code here...
  return input.filter( x => Number.isInteger(x)).map((s => (s % 2 === 0)?'even':'odd'));
};
