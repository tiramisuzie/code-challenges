...
const addCurve = (arr) => {
  // Solution code here...
  const newArr = [];
  for (let scoreKey in arr){
    const score = arr[scoreKey];
    newArr.push( (score * 1.05) );
  }
  return newArr;
}
...
const removeWithForEach = (input, callback) => {
  // Solution code here...
  input.forEach(function(x){callback(x,input);});
  return input;
}
...
describe('Testing challenge 3', () => {
  test('It should increase each raw score by 5%', () => {
    ...
    // expect(addCurve([55, 79, 100, 85, 92])).toBeCloseTo([ 57.75, 82.95, 105, 89.25, 96.60000000000001 ]);
  });
});
