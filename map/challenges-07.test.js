...
function myMap(arr, fun) {
  ...
  //  applies the fun
  //  puts the result of the fun in to our answer array
...
const forEachTwoToThe = (input) => {
  // Solution code here...
  const ans = [];
  input.forEach(input => {ans.push(Math.pow(2, input))});
  return ans;
};
...
const mapTwoToThe = input => input.map( x => Math.pow(2, x));
  // Solution code here...
...
const charCode = input => input.map(string => string.charCodeAt(0));
  // Solution code here...
...
const extractStats = (stats) => {
  // Solution code here...
  return stats.map(s => {
    return {name: s.stat.name, total: s.effort + s.baseStat}
  });
}
