...
const totalSumCSV = (input) => {
  ...
  for (let i=0; i<separated.length; i++){
    let input = total += parseInt(separated[i]);
  }
  return total;
};
...
const listFoods = (recipe) => {
  ...
  for (let i=0; i<gruffaloCrumble.ingredients.length; i++){
    let index = gruffaloCrumble.ingredients[i].indexOf(' ');
    let sliced = gruffaloCrumble.ingredients[i].slice(index+1);
    let indexTwo = sliced.indexOf(' ');
    let res = sliced.slice(indexTwo+1);
    result.push(res);
  }
